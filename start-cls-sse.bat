...
echo ====================================
echo 启动 SuperBizAgent（腾讯云 CLS - SSE 模式）
echo ====================================
echo.
...
REM 启动腾讯云 CLS MCP（SSE 模式，需先安装 Node.js）
echo [信息] 启动腾讯云 CLS MCP（SSE 模式）...
REM 需要在 mcp_servers/.env 中配置 TENCENTCLOUD_SECRET_ID 和 TENCENTCLOUD_SECRET_KEY
start "CLS MCP (Tencent)" cmd /c "cd /d !PROJECT_ROOT!\mcp_servers && npx -y cls-mcp-server@latest"
...
echo CLS 模式: 腾讯云 CLS（SSE，端口 3000）
