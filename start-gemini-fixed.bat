...
REM 启动 CLS MCP（修复版）
REM 问题原因：原脚本第60行 "cd mcp_servers && .venv\Scripts\python.exe"
REM cd 进入 mcp_servers 后，.venv 路径就变成了 mcp_servers/.venv，找不到虚拟环境
REM 修复：使用绝对路径，先 cd 到 mcp_servers 目录，但用项目根目录的虚拟环境来执行
echo [信息] 启动 CLS MCP ... 
start "CLS MCP" cmd /c "cd /d !PROJECT_ROOT!\mcp_servers && !PROJECT_ROOT!\.venv\Scripts\python.exe cls_server.py"
ping 127.0.0.1 -n 4 >nul

REM 启动 Monitor MCP（这个原脚本没问题的，但改为绝对路径更可靠）
